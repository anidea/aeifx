...
struct FX200Configuration
{
	...
	FX200Configuration(RFIDReadStyle readStyle, uint8_t pin, HardwareSerial* fx200Serial, uint16_t readTime = 400, uint16_t expireTime = 2000)
		: readStyle(readStyle),
		pin(pin),
...
public:
	...
	void setSolvedTag(byte index, const char* data);
	bool isSolved();
...
public:
	...
	void setSolvedTag(const char* data)
	{
		strcpy(solutionBuffer, data);
	}
private:
	char tagBuffer[32];
...
template <int count>
void FX200<count>::setSolvedTag(byte index, const char* data)
{
	if (index >= count) return;

	tagData[index].setSolvedTag(data);
}
...
{
	static_assert(count > 0, "Must have at least one FX200 reader!");
